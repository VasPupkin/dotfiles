...
local tooltip
local state = {}
-- local current_day_format = "<u>%s</u>" --original
local current_day_format = "<span color='#ff0000' underline='single' weight='bold' size='large'>%s</span>" --шрифт для сегодняшнего дня
local header_format = "<span color='orange' font='DejaVu Sans' weight='bold' size='large'>%s</span>" --шрифт для заголовка
function displayMonth(month,year,weekStart)
	...
	for x=0,6 do
		lines = lines .. os.date(" %a ",os.time{year=2006,month=1,day=x+wkSt}) --строка дней недели
	end

	lines = lines .. "\n" .. os.date(" %V",os.time{year=year,month=month,day=1}) --столбец номера недели
	...
	while writeLine < (stDay + 1) do
        ...
        local header = string.format(header_format, mylib.align_center((os.date("%B %Y\n",os.time{year=year,month=month,day=1})), 44))
